################################################################################
## Constant Speed Multi-Layer Frontal Polymerization Printing - WITH SUBSTRATE
## WITH EXTRA INITIATION INK AND AIR BUFFER (SHORT VERSION - 5mm layers)
## NO PID CONTROL - constant 0.8 mm/s printing speed
## Fine mesh, no adaptivity (v3 approach)
##
## Domain:
##   - Glass substrate: 6.8mm x 3.3mm (x = -1.6mm to 5mm, y = -3.3mm to 0)
...
##
## Boundary Conditions:
##   - Bottom of glass: 85°C (Dirichlet)
##   - Top of ink: convection to ambient (20°C) via ink_top_moving
##   - ink_top_moving automatically captures all ink-buffer interfaces
##   - No convection on ink bottom (in contact with substrate or lower layers)
##
## Printing Process:
...
[Mesh]
  ...
  # Fine mesh (2x), no adaptivity (v3 approach)
  [gen]
    ...
    ny = 170                  # 2x finer: 50 for substrate + 96 for ink + 4 for top buffer
  []

  # Glass substrate region (block 2) - extends under buffer on right
  [substrate]
    type = SubdomainBoundingBoxGenerator
    input = 'gen'
    block_id = 2
    bottom_left = '-0.0016 -0.0033 0'
    top_right = '0.0052 0 0'
  []
  ...
  # Air buffer on right side (x=5mm to 5.2mm, y=0 to 6.6mm) - block 4
  [buffer_right]
    type = SubdomainBoundingBoxGenerator
    input = 'air'
    block_id = 4
...
[Variables]
  # Temperature on substrate and ink only
...
[BCs]
  ...
  #   - Right edge of ink when it reaches x=20mm (ink-buffer interface)
...
[Functions]
  # Constant speed velocity function with layer transitions (SHORT VERSION - 5mm layers)
  # Nozzle starts at x=0 (right edge of extra initiation ink, start of main domain)
...
# v3: Adaptivity DISABLED - using fine mesh instead to avoid air pocket issues
# [Adaptivity]
#   marker = errorfrac
#   max_h_level = 4
#   [Indicators]
...
[Outputs]
  [exodus]
    type = Exodus
    file_base = constant_speed_0p8_substrate_MultiLayer_out/constant_speed_0p8_substrate_MultiLayer_out
    time_step_interval = 5      # only output every 10 timesteps
    execute_on = 'initial timestep_end'
  []
  [csv]
    type = CSV
    file_base = constant_speed_0p8_substrate_MultiLayer_out/constant_speed_0p8_substrate_MultiLayer_data
    execute_on = 'initial timestep_end'
  []
[]
