################################################################################
## Constant Speed Single-Layer Frontal Polymerization Printing - INSULATIVE SUBSTRATE
## NO PID CONTROL - constant 0.8 mm/s printing speed
##
## Domain:
##   - No substrate domain (insulative substrate assumption)
...
##
## Boundary Conditions:
##   - Bottom of ink: ADIABATIC (no heat flux - insulative substrate)
...
[Mesh]
  ...
  # Air region (rest of domain) - block 0
  [air]
    type = SubdomainBoundingBoxGenerator
    input = 'ink'
    block_id = 0
  ...
  # Note: No bottom buffer, so moving boundary only captures top surface
...
[BCs]
  # Bottom of ink: ADIABATIC (no BC = zero flux for insulative substrate)
  # No bottom BC needed - natural boundary condition is zero flux
...
[Postprocessors]
  ...
  [nozzle_x_value]
    ...
    expression = '0.0016 + nozzle_displacement'
...
[Controls]
 ...
 [pid_velocity]
    type = PIDTransientControlIntervalNewReport
    ...
    # NEW: Add these two lines to connect the postprocessors
    control_interval_pp = control_interval_out
    dt_control_pp = dt_control_out
  []
  [bcs]
    type = TimePeriod
...
[Outputs]
  [exodus]
    type = Exodus
    file_base = pid_1_insulativesubstrate_SingleLayer_out/pid_speed_1_insulativesubstrate_SingleLayer_out
    execute_on = 'initial timestep_end'
    time_step_interval = 10
  []
  [csv]
    type = CSV
    file_base = pid_1_insulativesubstrate_SingleLayer_out/pid_speed_1_insulativesubstrate_SingleLayer_data
    execute_on = 'initial timestep_end'
  []
[]
