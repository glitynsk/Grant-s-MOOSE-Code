################################################################################
## Constant Speed Multi-Layer Frontal Polymerization Printing - INSULATIVE SUBSTRATE
...
##   - Air buffer layer (block 4): surrounds print area on right, top, and left (NO BOTTOM)
##     (allows ink_top_moving to capture top ink-air interfaces only)
##   - Total domain: 6.8mm x 6.6mm (with 0.2mm buffer on right, top, left)
##
## Block IDs:
...
##   - ink_top_moving automatically captures top ink-buffer interfaces only
##   - Bottom of first layer ink: ADIABATIC (insulative substrate)
##
## Printing Process:
...
[Mesh]
  # Extended rectangular mesh with air buffer layer around print area (NO BOTTOM BUFFER)
  # Buffer allows ink_top_moving to capture top ink-air interfaces only
  # Bottom is adiabatic (insulative substrate)
  [gen]
    type = GeneratedMeshGenerator
    dim = 2
    xmin = -0.0016            # Extra 1.6mm for initiation ink
    xmax = 0.0052             # 5 mm + 0.2mm buffer on right
    ymin = 0                  # NO bottom buffer - ink starts at y=0
    ymax = 0.0066             # Top of 4 ink layers (6.4 mm) + 0.2mm buffer
    nx = 84                   # 2x finer: 20 for extra + 60 for main + 4 for buffer
    ny = 120                  # 2x finer: 96 for ink + 4 for top buffer (NO bottom buffer)
  []
  ...
  # NO bottom buffer - insulative substrate (adiabatic BC)
...
[Outputs]
  [exodus]
    type = Exodus
    file_base = constant_speed_0p8_insulativesubstrate_MultiLayer_out/constant_speed_0p8_insulativesubstrate_MultiLayer_out
    time_step_interval = 5
    execute_on = 'initial timestep_end'
  []
  [csv]
    type = CSV
    file_base = constant_speed_0p8_insulativesubstrate_MultiLayer_out/constant_speed_0p8_insulativesubstrate_MultiLayer_data
    execute_on = 'initial timestep_end'
  []
[]
