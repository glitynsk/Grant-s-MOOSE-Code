################################################################################
## Multi-Layer PID-Controlled Frontal Polymerization Printing
## Based on kp10.i with extensions for 4-layer printing
##
## Layer Transition Logic:
##   1. When front reaches within transition_gap (1mm) of layer end
##   2. Nozzle moves up (vertical_delay = 1s)
##   3. Print at initial_velocity (1mm/s) for initial_nozzle_offset (7mm)
...
# Parameters documented in header comments (defined in Controls block)
...
[Mesh]
  [gen]
    ...
    xmin = 0.0
    ymin = 0.0
    xmax = 0.005           # 20 mm length
    ymax = 0.0064         # 6.4 mm height (4 layers × 1.6 mm)
    nx = 30             # Same resolution as kp10.i
...
[Functions]
  ...
  # Using floor formula instead of mod: layer - 2*floor(layer/2) = 0 for even, 1 for odd
  [direction_func]
    type = ParsedFunction
...
[Postprocessors]
  ...
  # Layer 1,3: end at x=0.005; Layer 2,4: end at x=0
  # Use: current_layer - 2*floor(current_layer/2) to get odd/even (0=even, 1=odd)
  [front_to_layer_end]
    type = ParsedPostprocessor
...
[Controls]
  # Multi-layer PID velocity control
  [multilayer_pid]
  
    pid_stop_distance = 0.001          # 1 mm
    finishing_velocity = 0.001         # 1 mm/s
    ...
    initial_velocity = 0.005       # 10 mm/s
    initial_nozzle_offset = 0.001 # 6.2 mm (from 0.8mm to 7mm)
    initial_nozzle_x = 0.0008     # 0.8 mm (center of initial 1.6mm ink)

    # Layer geometry
    layer_length = 0.005           # 20 mm
...
[Outputs]
  [exodus]
    type = Exodus
    file_base = multilayer_pid_ci0p1_final_old/multilayer_pid_ci0p1_out
    execute_on = 'initial timestep_end'
    time_step_interval = 10
  []
  [csv]
    type = CSV
    file_base = multilayer_pid_ci0p1_final_old/multilayer_pid_ci0p1_data
    execute_on = 'initial timestep_end'
  []
[]
